document.addEventListener('DOMContentLoaded', function () {
    ...
    var A11width = 960 - A11margin.left - A11margin.right;
    var A11height = 500 - A11margin.top - A11margin.bottom;
    
    var A11g = A11svg
      .append('g')
      .attr(
        'transform',
        'translate(' + A11margin.left + ',' + A11margin.top + ')',
      );
    
    var A11parseTime = d3.timeParse('%Y%m%d');
    
    var A11x = d3.scaleTime().range([0, A11width - A11margin.right]);
    var A11y = d3.scaleLinear().range([A11height, 0]);
    var A11z = d3.scaleOrdinal(d3.schemeCategory10);
    ...
    d3.tsv('data.tsv', function (d) {
      ...
      let A11columns = ['date', 'New York', 'San Francisco', 'Austin'];
      for (d of data) {
        d.date = A11parseTime(d.date);
        for (var i = 1, n = A11columns.length, c; i < n; ++i)
          d[(c = A11columns[i])] = +d[c];
      }
      ...
      A11svg
        ...
        .attr('y', 15)
        .attr('dy', '0.35em')
        .attr('id', (d) => d.id.substring(0, 3).toUpperCase() + '_label')
